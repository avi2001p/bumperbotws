...
def generate_launch_description():
    ...
    # Each 0.01 less = +0.02 m (2 cm) semicircle diameter. 0.02 = +6 cm vs the
    # original 0.05, leaving ~3 cm from the wall. Bump up if it scrapes.
    wall_clearance_arg = DeclareLaunchArgument("wall_clearance", default_value="0.02")
    # How far each turn completes: 1.0 = full 180 (keeps the return straight
    # aligned). Lower it if the robot over-turns. COVERAGE geometry, NOT PID.
    arc_fraction_arg = DeclareLaunchArgument("arc_fraction", default_value="1.0")
    ...
    coverage = Node(
        ...
        parameters=[{
            ...
            "arc_complete_fraction": LaunchConfiguration("arc_fraction"),
        }],
    )
